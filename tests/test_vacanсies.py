...
def test_description_concat():
    ...
    item = {"snippet": {"requirement": "Знание Python и Django", "responsibility": "Разработка веб‑приложений"}}
    vacancy = Vacancy.from_hh_api(item)
...
def test_description_missing_requirement():
    ...
    item = {"snippet": {"responsibility": "Поддержка существующих сервисов"}}
    vacancy = Vacancy.from_hh_api(item)
...
def test_description_missing_responsibility():
    ...
    item = {"snippet": {"requirement": "Опыт работы с SQL"}}
    vacancy = Vacancy.from_hh_api(item)
...
def test_published_at_parsing():
    ...
    item = {"published_at": "2025-12-15T10:30:00+0300", "name": "Test Job", "alternate_url": "https://test.ru"}
    vacancy = Vacancy.from_hh_api(item)
...
def test_published_at_invalid_format():
    ...
    item = {"published_at": "не-дата", "name": "Invalid Date Job", "alternate_url": "https://invalid.ru"}
...
def test_employer_parsing():
    ...
    item = {"employer": {"name": "ООО Новый Проект"}, "name": "Dev", "alternate_url": "https://url.ru"}
    vacancy = Vacancy.from_hh_api(item)
...
def test_employer_missing():
    ...
    item = {"name": "Freelance Job", "alternate_url": "https://freelance.ru"}
    vacancy = Vacancy.from_hh_api(item)
...
def test_id_in_from_hh_api():
    ...
    item = {"id": "98765", "name": "Job with ID", "alternate_url": "https://id.ru"}
    vacancy = Vacancy.from_hh_api(item)
